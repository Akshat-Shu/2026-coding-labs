...
With the grid flat, `grid[next_index]` just needs `next_index`, there's no reason to keep computing `next_row`/`next_col` at all. Replaced the `drow[4]` / `dcol[4]` pair with a single `offsets[4] = {-cols, cols, -1, 1}` and dropped the `current_index / cols` and `current_index - current_row * cols`. Each direction is now one add. After:
...
Here are some optimizations that looked promising but ended up being not worth it.

- **OpenMP on `compute_congestion_pressure`.** Added `#pragma omp parallel for schedule(static)` over the row loop, and tried both the naive per-pass form and the hoisted "single `parallel` region outside the pass loop" form, with 2, 4, and 8 threads. Slower at every configuration, ~30 ms worse than serial. The reason is granularity: each pass is only about 25 μs of actual work, so splitting it across 8 threads gives each thread ~3 μs of compute, while the per-pass barrier cost is ~5-10 μs. Times 4,096 passes, the synchronization overhead exceeds the parallelism win. OpenMP would help here only if the grid were significantly larger (so each pass had more work to amortize the barrier) or if I parallelized across route requests instead, but those share the `heatmap` and would need atomics or per-thread heatmaps + a reduction step. Not worth the refactor for this size workload.

- **Row pointers inside `compute_congestion_pressure`.** Tried keeping `cur`, `north`, `south`, `east`, `west` as raw pointers and bumping each by 1 per column iteration instead of recomputing `cur_data[index ± cols ± 1]`. Was slightly *slower* (around 1.55 s vs ~1.46 s at that stage). The compiler at `-O3` is already doing strength reduction on the indexed accesses, turning `cur_data[index]` and `cur_data[index - cols]` into pointer increments under the hood. Doing it by hand just adds register pressure and confuses the alias analysis without giving the compiler anything new.

- **`vector<bool>` for `visited`.** Replaced the `unsigned char` visited array with `std::vector<bool>` to halve the cache footprint. Was slower (~0.77 s) than `vector<unsigned char>` at the same stage. `vector<bool>` is the famous special case in the standard library, it's bit-packed and returns a proxy reference, so each read/write becomes a load-byte + bit-shift + mask + (for writes) read-modify-write. The per-access instruction overhead outweighed the cache savings, and the proxy reference also blocked some of the optimizations the compiler applied to a plain byte array.

- **Bitmap (1 bit per cell) for `grid`.** Same idea as `vector<bool>`-for-visited, applied to the grid itself. Tried `std::array<uint8_t, (kRows*kCols + 7) / 8>` with manual `(grid[i >> 3] >> (i & 7)) & 1` checks. Slower (~0.53 s vs ~0.49 s). The grid is read ~131 million times in BFS, and replacing the simple byte-load + compare with load + shift + AND + compare added a couple of cycles per check. The original `char` grid (67 KB) already fits between L1 and L2 and is read in a predictable enough pattern that the hardware prefetcher hides most of the latency. The arithmetic to extract a bit per cell cost more than the cache footprint saved.

The pattern across these: each one is a real, plausible-sounding optimization that fails on this specific workload for a specific reason, small per-region work for OpenMP, compiler already doing strength reduction for the row pointers, proxy overhead for `vector<bool>`, and the original char-grid not actually being cache-bound for the bitmap.
...
Same 1,200 route requests, same 4,096 congestion passes, same checksums. ~6× faster end-to-end (~2.16 s to ~0.35 s).
...
- **Q4.2: Why use `gprof` if `perf`/FlameGraph already show hotspots?**
  - Different views. `perf` samples an optimized binary, which is what you actually ship, best for checking where the time is really going. `gprof` runs an instrumented `-O0` binary, which is artificial, but gives you exact call counts and a readable call graph. Helpful for understanding structure, especially if a small helper is called a million times and you want to know that.
...
- **Q5.1: Valgrind Memcheck vs AddressSanitizer? When to use each?**
  ...
  - Day to day: ASan. Special cases (digging deep into things): Valgrind.
